...
docker run --name jms_all --network jms_net --rm \
  ...
  --privileged=true \
  jumpserver/jms_all:v2.25.3 init_db
...
docker run --name jms_all --rm \
  ...
  jumpserver/jms_all:v2.25.3 init_db   # 确定无报错
```
...
# 拉取新版本镜像
docker pull jumpserver/jms_all:v2.25.3
...
docker run --name jms_all --rm \
  ...
  -e REDIS_PORT=6379 \
  -e REDIS_PASSWORD=****** \
  jumpserver/jms_all:v2.25.3 upgrade     # 确定无报错
